sap.ui.define([
	...
	return Controller.extend("ZSD_MAKINA_SEVK.ZSD_MAKINA_SEVK.controller.Main", {
		...
		onInit: function () {
			...
			//15012020 Sevkiyat Noktası parametresi eklendiği için 
			//sevkiyat noktası çekildikten sonra çekilecek.
			//this.methodGetBelgeNoShListSet(oModelData, this.jsonMainModel);
			this.getSevkNok();
		},

		getSevkNok: function () {
			//Kullanıcı üzerinde ki sevkiyat noktasını okuyacağız.
			var oMainModel = this.getView().getModel("mainView");

			var uName = "";
			var sPath = "GetUserSevkNokSet(IUname='" + uName + "')";
			...
			oModel.read(sPath2, {
				success: function (oData) {
					if (oData !== undefined) {
						dialogBusy.close();
						if (oData.EError !== "X") {
							oMainModel.setProperty("/sevkNok", oData.EVstel);
							that.methodGetBelgeNoShListSet();
						} else {
		...
		methodGetBelgeNoShListSet: function () {
			var oModel = this.getOwnerComponent().getModel();
			...
			oModel.read("/EtTeslimAlanShSet", {
				success: function (resp) {
					...
					if (resp.results.length > 0) {
						...
						that.methodGetSevkNokShListSet(oModel, jsonModel);
					} else {
						MessageToast.show("Bir Hata Oluştu");
					}
				},
		...
		methodGetSevkNokShListSet: function (oModel, jsonModel) {
			oModel.read("/EtSevkNokShSet", {
				success: function (resp) {
					dialogBusy.close();
					if (resp.results.length > 0) {
						jsonModel.setProperty("/sevkNokSh", resp.results);
					} else {
						MessageToast.show("Bir Hata Oluştu");
					}
				},
		...
		methodGetTeslimatListSet: function () {
			...
			var sevkNok = jsonModel.getProperty("/sevkNok");
			...
			if (belgeNo) {
				...
				f.push(filter);
				if (sevkNok) {
					filter = new Filter("IvSevkNok", sap.ui.model.FilterOperator.EQ, sevkNok);
					f.push(filter);
				}
			} else {
			...
			oModel.read("/EtTeslimatListSet", {
				...
				success: function (resp) {
					...
					if (resp.results.length > 0) {
						...
						that.methodClearTableSelection(jsonModel);
					} else {
						MessageToast.show("Uygun Teslimat Bulunamadı");
					}
				},
				error: function (resp) {
					dialogBusy.close();
		...
		methodSevkNokSelected: function (oEvent) {
			var jsonModel = this.getView().getModel("mainView");
			var oItem = oEvent.getSource();
			var teslimAlan = oItem.getCells()[0].getText();
			jsonModel.setProperty("/sevkNok", teslimAlan);
			this.closeDialog();
		},
		...
		onValueHelpRequestSevkNok: function (oEvent) {
			this.openDialog("ZSD_MAKINA_SEVK.ZSD_MAKINA_SEVK.fragment.sevkNoktasi");
		},
